...
export default {
  ...
  mounted() {
    ...
    this.$axios.get(`${process.env.VUE_APP_BASE_URL}/Ala/CheckHistory`).then(
      result => {
        //console.log(result)
        ...
        // this.checkHistory.forEach((item, index) => {
        //   Data.unshift(item);
        // });

        // this.setCheckHistory(Data);

        // console.log(this.checkHistory);

        //   this.setCheckHistory(Data)
        if(this.checkHistoryIsFirst){
         
        let temp=[]
        temp=this.checkHistory
        temp.push.apply(temp,result.data.data)
        this.setCheckHistory(temp)
        this.dataList = this.checkHistory
       
        //设置为false
        this.setCheckHistoryIsFirst(false)
        }
